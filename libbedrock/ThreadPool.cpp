...
pthread_key_t  beckrock::G_WORKER_KEY ;
...
void ThreadPool::enqueueSentinel(bedrock::callback::Functor* cb){
	RefCountedSentinel* r = new RefCountedSentinel(_worker.size(),cb);
	for(vector<Worker*>::iterator it = _workers.begin();it!=_workers.end();it++){
		(*it)->enqueue(callback::wrap(r,&RefCountedSentinel::releaseReference),true);
	}
}
...
int ThreadPool::getNextThreadID(void){
	...
	int result = (_current_work++) % _workers.size();
...
ThreadPool::Worker::Worker(ThreadPool* owner,int threadkey,bool active):
	...
	int err;
	err = pthread_create(&_thread,NULL,ThreadPool::Worker::run,NULL);
	if(err!=0){
...
void ThreadPool::Worker::enqueue(bedrock::callback::Functor* cb,
                                 bool delete_functor){
	_lock.lock();

	FunctorHolder fh = {cb,delete_callback};
