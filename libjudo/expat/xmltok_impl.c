...
static
size_t PREFIX(scanPercent)(const ENCODING *enc, const char *ptr, const char *end,
			const char **nextTokPtr)
{
  if (ptr == end)
    return XML_TOK_PARTIAL;
...
static
size_t PREFIX(scanPoundName)(const ENCODING *enc, const char *ptr, const char *end,
			  const char **nextTokPtr)
{
  if (ptr == end)
    return XML_TOK_PARTIAL;
...
static
size_t PREFIX(scanLit)(int open, const ENCODING *enc,
		      ...
		      const char **nextTokPtr)
{
  ...
  switch (BYTE_TYPE(enc, ptr)) {
  case BT_QUOT:
    return (int)PREFIX(scanLit)(BT_QUOT, enc, ptr + MINBPC(enc), end, nextTokPtr);
  case BT_APOS:
    return (int)PREFIX(scanLit)(BT_APOS, enc, ptr + MINBPC(enc), end, nextTokPtr);
  case BT_LT:
    {
      ptr += MINBPC(enc);
      if (ptr == end)
  ...
  case BT_S: case BT_LF:
    ...
    *nextTokPtr = ptr;
    return XML_TOK_PROLOG_S;
  case BT_PERCNT:
    return (int)PREFIX(scanPercent)(enc, ptr + MINBPC(enc), end, nextTokPtr);
  case BT_COMMA:
    *nextTokPtr = ptr + MINBPC(enc);
  ...
  case BT_NUM:
    return (int)PREFIX(scanPoundName)(enc, ptr + MINBPC(enc), end, nextTokPtr);
#define LEAD_CASE(n) \
  case BT_LEAD ## n: \
    if (end - ptr < n) \
      return XML_TOK_PARTIAL_CHAR; \
...
#undef LEAD_CASE
    ...
    case BT_PERCNT:
      if (ptr == start) {
	size_t tok =  PREFIX(scanPercent)(enc, ptr + MINBPC(enc),
				       end, nextTokPtr);
	return (int)((tok == XML_TOK_PERCENT) ? XML_TOK_INVALID : tok);
      }
      *nextTokPtr = ptr;
      return XML_TOK_DATA_CHARS;
    case BT_LF:
