...
static size_t nextScaffoldPart(XML_Parser parser);
...
#define poolLength(pool) ((unsigned)((pool)->ptr - (pool)->start))
...
int XML_Parse(XML_Parser parser, const char *s, size_t len, int isFinal)
{
  if (len == 0) {
    ...
    nLeftOver = s + len - end;
    if (nLeftOver) {
      if ((buffer == 0) || (nLeftOver > (unsigned)(bufferLim - buffer))) {
...
{
  if (len > (unsigned)(bufferLim - bufferEnd))
  {
    /* FIXME avoid integer overflow */
    size_t neededSize = len + (unsigned)(bufferEnd - bufferPtr);
    ...
    if (neededSize  <= (unsigned)(bufferLim - buffer)) {
#ifdef XML_CONTEXT_BYTES
...
{
  if (eventPtr)
    return (long)(parseEndByteIndex - (parseEndPtr - eventPtr));
  return -1;
}

size_t XML_GetCurrentByteCount(XML_Parser parser)
{
  if (eventEndPtr && eventPtr)
    return (unsigned)(eventEndPtr - eventPtr);
  return 0;
}
  ...
  for (;;) {
	...
	*eventEndPP = s;
	...
	if (!groupConnector)
      ...
      if (dtd.in_eldecl) {
	size_t myindex = nextScaffoldPart(parser);
	if (myindex < 0)
	  return XML_ERROR_NO_MEMORY;
...
int poolGrow(STRING_POOL *pool)
{
  if (pool->freeBlocks) {
    ...
    if ((unsigned)(pool->end - pool->start) < pool->freeBlocks->size) {
...
build_model (XML_Parser parser)
{
  ...
  XML_Content *cpos;
  char * str;                              
  size_t allocsize = dtd.scaffCount * sizeof(XML_Content) + dtd.contentStringLen;
