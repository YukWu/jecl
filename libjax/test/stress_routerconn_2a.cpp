...
// Filename:      stress_routerconn2a.cpp
// Description:   tester
...
// 
// $Id: stress_routerconn_2a.cpp,v 1.1 2002/04/30 11:39:36 akuma Exp $
//============================================================================
#include "jax.h"
using namespace jax;
...
bedrock::Mutex        G_continue_lock;
bedrock::CondVariable G_continue_signal(G_continue_lock);
...
public:
    Connector(bedrock::net::Address& a, bool outgoing)
	  ...
	  _name("component2"), _secret("thepassword"), _outgoing(outgoing)
        {
    ...
    void onRouterConnected()
        {
            MutexProxy mp(G_continue_lock);
            G_continue_signal.signal();
        }
    ...
    void onRouterPacket(Packet* p)
        {
            cerr << "Got router packet: " << p->toString() << endl;
        }
};
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        MutexProxy mp(G_continue_lock);
        G_continue_signal.wait();

        cerr << "Beginning blast cycle." << endl;
        // Begin blasting packets
        while (1)
        {
            judo::Element* elem = new judo::Element("message");
            elem->putAttrib("to", "component1");
            elem->putAttrib("from", "component3");
            elem->addElement("ID", "aldkjfalksdjfalsdjkf");
            c->deliver(new jax::Packet(elem));
        }
    }
